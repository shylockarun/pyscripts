...
parsedcoauth = ((int(row[0]),int(row[1]),int(row[2])) for row in coauthread)
...
edgelist2 = []
cs = csv.writer(open('blah.csv','w',newline=''))
cs.writerow(['source','target'])
for k,l,m in parsedcoauth:
  if m <= 1998:
    m = 0
  else:
    m = m - 1998
